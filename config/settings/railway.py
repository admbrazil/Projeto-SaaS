...
# --- Middleware ---
# Remove middlewares que dependem de infra não disponível no Railway
_EXCLUDED = {
...
if _sentry_dsn:
    ...
    sentry_sdk.init(dsn=_sentry_dsn, integrations=[DjangoIntegration()], traces_sample_rate=0.1, send_default_pii=False)

# --- Cache ---
_redis_url = os.environ.get("REDIS_URL", "")
if _redis_url and not _redis_url.startswith("$"):
    CACHES = {"default": {"BACKEND": "django_redis.cache.RedisCache", "LOCATION": _redis_url, "OPTIONS": {"CLIENT_CLASS": "django_redis.client.DefaultClient"}}}
else:
